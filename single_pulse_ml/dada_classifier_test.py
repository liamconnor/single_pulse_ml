...
def dedisperse(data, dm=56.8):
    data = data.copy()

    datam = data.mean(0).mean(0)
    ind = np.where(np.abs(datam - np.median(datam))>3*np.std(datam))[0]
    data[..., ind] = 0.

    dt = 0.00008192
    nfreq = 1536
    freq = np.linspace(1550., 1250., nfreq)
    tdelay = 4.148e3*dm*(freq**-2 - freq[0]**-2)
    ntime = len(data[0])
    ...
    for tab in range(12):
        for ii, f in enumerate(freq):
            data[tab, ii] = np.roll(data[tab, ii], np.int(ntime//2-tdelay[ii]/dt))

        maxind = np.argmax(data[tab].mean(0))
        maxind_arr.append(maxind)

    return data, maxind_arr

data_classify = np.empty([ntab, 32, 64])
counter = -1
for page in reader:
    counter += 1
